...
§10–11의 리타게팅된 human 시연을 **실제 로봇 파운데이션 모델(RLWRLD RLDX-1)이 먹는 포맷(LeRobot v2.1 + `modality.json`)** 으로 직렬화하고, 포맷 문서가 아니라 **RLDX-1 저장소의 실제 학습 로더로 검증(LOADER_OK)** 했다 — 품질 리포트(관절한계 여유 min 28°, jerk<0.0008, IK<0.1mm, 16→12 필터)와 대시보드 포함.

**전체 내용은 RLDX 전용 페이지로 이동·정리했다** → **[RLDX-1 실전 §4](rldx.md)** (스키마 표, 품질 리포트, 로더 검증 로그, 정직 캐비엇 전문). RLDX-1 자체의 실측 A/B·아키텍처 리뷰도 같은 페이지와 [리뷰](reviews/rldx1.md)에서.
