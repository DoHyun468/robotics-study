...
GPT (언어)  ──→  Decision Transformer (2021)   행동 = return 조건 다음-토큰 예측
             ...
             └→  IRIS (2023)                    관측을 토큰화 → GPT world model 안에서 상상 학습
                          │
             (이후 VLA의 action 토큰화, Genie·Cosmos의 자기회귀 동역학으로 이어지는 발상의 원류)
```
...
- [IRIS (2023)](iris.md) — 프레임을 VQ 토큰 16개로 바꿔 GPT world model을 만들고, 그 상상 안에서 정책 학습. Atari 100k 샘플효율.

## 비교표

| 모델 | 연도 | 무엇을 토큰화하나 | 행동 결정 | 대표 수치 (원문 대조) |
|------|------|---------------------|-----------|----------------------|
...
| [IRIS](iris.md) | 2023 | **관측 프레임 자체** (VQ 16토큰/프레임, 어휘 512) | GPT 상상 롤아웃(H=20)에서 배운 actor-critic | Atari 100k mean HNS 1.046, 10/26게임 초인간 |
...
- 이후 계보와의 연결: action을 토큰으로 다루는 발상은 [OpenVLA](../reviews/openvla.md)의 action 토큰화로, 자기회귀 시퀀스 생성으로 세계를 굴리는 발상은 [Genie](genie.md)·[Cosmos](cosmos.md)의 AR world model로 이어진다. DT·TT는 Dreamer류([Latent](latent.md))와 달리 **상상 롤아웃으로 정책을 개선하지 않고** 데이터 궤적 분포를 조건부로 재생한다 — 오프라인 RL에 자연스럽고, 분포 밖 개선에는 약하다. 한편 [IRIS](iris.md)는 반대로 **토큰 시퀀스 모델을 world model로 삼아 상상 학습**을 한다 — 시퀀스라는 표현은 같지만 쓰임(정책 재생 vs 세계 시뮬레이션)이 갈리는 지점.
