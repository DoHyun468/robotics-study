...
수치는 원문(arXiv 2301.04104) 대조 완료. **하나의 하이퍼파라미터 세트**로 8개 도메인 150+ 태스크 전부를 커버한 결과:

| 도메인 (규모) | DreamerV3 결과 (원문) |
|---------------|----------------------|
| Proprio Control Suite (18태스크, 500K 스텝) | median **845.5** vs D4PG 787.2 — SOTA |
| Visual Control Suite (20태스크, 1M 스텝) | DrQ-v2·CURL 능가 — SOTA |
| Atari 200M (55게임) | median **302%** vs DreamerV2 219% — Rainbow·IQN 능가 |
| Atari 100k (26게임, 400K 스텝) | 동일 하이퍼로 경쟁력 유지 |
| BSuite (23환경 468구성) | 카테고리 mean **0.627** vs Muesli 0.537 — SOTA |
| Crafter (1M 스텝) | score **14.5±1.6%** vs DreamerV2 10.0±1.2% — SOTA |
| DMLab (8태스크) | IMPALA의 최종 성능을 **50M vs 10B 스텝**으로 도달 (~130배 데이터 효율) |
| **Minecraft (100M 스텝)** | **40시드 중 24시드가 다이아몬드 채굴**(총 50 에피소드, 최초 채굴 29M 스텝) — 사람 데이터·커리큘럼 없이 최초 |

**스케일링**: XS(8M)~XL(200M) 5개 크기에서, 클수록 최종 성능과 **데이터 효율이 함께 단조 상승** — "하나의 레시피로 크기만 키우면 된다"는 주장 그대로.
