...
**등장인물 4개.** 전부 $4{\times}4$ 강체변환(SE(3))이고, 좌표계는 base(로봇 바닥, world), gripper(그리퍼), cam(카메라), marker(그리퍼에 붙인 ArUco) 넷이다:

| 기호 | 무슨 변환 | 어디서 오나 | 자세마다 변하나 |
|---|---|---|---|
| $G_i$ | gripper→base (자세 $i$) | **로봇 FK** — 조인트각에서 계산, 정확히 안다 | 변함 (팔을 움직이니까) |
| $C_i$ | marker→cam (자세 $i$) | **카메라 관측** — ArUco 검출+solvePnP | 변함 (마커가 그리퍼에 붙어 같이 움직임) |
| $X$ | cam→base | **구하려는 미지수** | 고정 (카메라는 삼각대처럼 붙박이) |
| $Y$ | marker→gripper 마운트 오프셋 | 미지수지만 관심 없음 → **소거** | 고정 (리지드 장착) |

즉 자세를 바꿀 때마다 "로봇이 아는 $G_i$" + "카메라가 본 $C_i$" 한 쌍씩 쌓이고, 그 뒤에 숨은 고정 미지수가 $X$와 $Y$다.
...
**⑤ 실전이라면 무엇이 달라지나 — 시뮬이 가려준 것들.** 이 실험은 FK가 완벽하고 렌즈 왜곡이 0이고 GT로 검증할 수 있는 시뮬이다. 실물에서 신경써야 하는 것:

- **회전 다양성이 최우선** — translation만 바꾸는 자세는 정보가 0이다($R_A{=}I$면 $(R_A-I)$가 통째로 0). 회전축 2개 이상을 ±30° 이상으로 흔드는 20~30자세가 표준.
- **FK 절대정밀도** — 조인트 영점 오차·백래시·그리퍼 무게에 의한 처짐이 $G_i$를 오염시키고 그대로 $X$로 전파된다. 로봇의 반복정밀도가 좋아도 **절대정밀도**는 mm급으로 나쁠 수 있다. [ROBOTIS 스프린트](robotis.md)에서 FK 피드백으로 droop을 보정한 것이 이 문제의 사촌.
- **단일 마커 → ChArUco 보드** — 평면 마커 하나는 depth 약제약(2단계에서 실측한 그 문제 — 여기 translation 4mm 잔차의 뿌리)과 정면 근처 pose ambiguity(두 해가 flip)가 있다. 코너 수십 개짜리 보드 + corner refinement + 비스듬한 각도 유지가 표준.
- **정지 후 촬영** — 움직이며 찍으면 FK 시점과 이미지 시점이 어긋난다. 자세마다 진동이 가라앉은 뒤 촬영.
- **닫힌형은 초기해로만** — 위 순차 풀이의 오차 전파를 재투영 오차 기준 비선형 정제(LM)로 마무리하고, 잔차 큰 쌍은 걸러내고 다시 푼다.
- **GT 없는 검증** — 실물엔 정답이 없으니 쌍별 $AX{-}XB$ 잔차 분포, $X$로 되쏜 재투영 픽셀 오차, 그리고 워크스페이스 여러 곳에서 카메라가 지정한 점을 로봇이 실제로 찍는 **터치 테스트**로 검증한다.
- **$X$의 수명** — 카메라를 건드리거나 온도·진동으로 마운트가 미세하게 돌면 무효가 된다. 주기적 재검증이 필요하다.

**⑥ eye-in-hand — 카메라를 그리퍼에 붙이면 미지수가 바뀐다.** 카메라가 그리퍼에 붙고 타깃 보드가 세상에 고정되면, 고정 미지수는 $X' = T_{c\to g}$(카메라 마운트)와 $W = T_{t\to b}$(보드 위치)가 된다. 구속은 "보드 점을 base로 보내는 경로가 모든 자세에서 같은 $W$"라는 것:

$$ W = G_i\,X'\,C_i = G_j\,X'\,C_j \;\;\Rightarrow\;\; \underbrace{(G_j^{-1}G_i)}_{A'}\,X' = X'\,\underbrace{(C_j\,C_i^{-1})}_{B'} $$

이번엔 그리퍼 상대운동이 **body-frame 묶음**($G_j^{-1}G_i$)으로 나온다 — ②에서 "반대로 묶으면 다른 미지수의 방정식이 된다"던 대칭의 정체가 이것이다. **AX=XB가 푸는 미지수는 항상 마운트 변환이고, 마운트가 움직이는 쪽(그리퍼)에 붙었으면 body-frame 묶음, 고정된 쪽(세상)에 붙었으면 base-frame 묶음이 된다.** 솔버 코드는 동일하고 쌍을 만드는 방향과 결과 해석만 다르다. `cv2.calibrateHandEye`가 eye-in-hand(cam→gripper) 기준으로 설계되어 eye-to-hand에서는 $G$를 뒤집어(base→gripper) 넣으라고 문서화된 것도 정확히 이 대칭이다.

실전 트레이드오프: eye-in-hand는 다가갈수록 해상도가 올라가 정밀 조작·비주얼 서보잉에 유리하지만 카메라가 움직이므로 모션블러·초점/노출 변화·케이블 처리가 생기고, eye-to-hand는 작업장 전역을 고정 시야로 보는 대신 팔 자체의 가림(occlusion)이 생긴다. 마운트와 보드 위치를 동시에 명시적으로 추정하는 $AX=YB$ 계열(robot-world hand-eye, Shah·Li)도 있다 — 우리 소거형과 달리 $Y$까지 결과로 나온다.
:::
